...
function App() {
  ...
  const listaErroGeo = [{ "codigo": 1, "texto": "Não foi dada permissão para o sistema encontrar sua localização" },
    ...
    async function obtemCidade(Latitude, Longitude) {
      ...
      await fetch(url)
        ...
        .catch(function (error) {
          console.error(`Não foi possível buscar a cidade a partir de lat/long. Erro: ${error.message}`)
        })
    }
  }, [])
  ...
  return (
    <> {/* React Fragment */}
      ...
      <Jumbotron>
        ...
        Aplicativo desenvolvido em ReactJS e integrado com a API OpenWatherMap.
        </p>
      </Jumbotron>
      {erroGeo &&
        ...
        {clima &&
          <Card bg="primary" className="text-center">
            <Card.Header>
              ...
              <h5>min: <strong></strong>{clima.main.temp_min}&#x2103;<FaArrowDown className="text-danger" />
                - máx: {clima.main.temp_max}&#x2103; <FaArrowUp className="text-success" /></h5>
            </Card.Header>
